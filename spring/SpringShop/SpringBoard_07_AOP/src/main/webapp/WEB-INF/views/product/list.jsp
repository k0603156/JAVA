	...
	<tbody>
		<c:choose>
			<c:when test="${list ne null && list.size() != 0 }">
				...
				<tr>
					<td>${pvo.pno }</td>
					<td><a href="/product/detail?pno=${pvo.pno }&pageNum=${pgvo.cri.pageNum }&amount=${pgvo.cri.amount }&type=${pgvo.cri.type }&keyword=${pgvo.cri.keyword }">${pvo.title }</a>
					<span class="badge badge-info">${pvo.commentcount }</span>
					</td>
